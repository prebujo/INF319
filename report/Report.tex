...
\begin{tabular}{l c l }
    ...
    $B_{v\alpha}            $   &-& distance for interval $\alpha$ and vehicle $v$ in cost 
                                    matrix $A_v$ column index                                   \\
    $Z_{v\beta}             $   &-& weight for interval $\beta$ for vehicle $v$ in cost matrix 
                                    $A_v$ row index                                             \\ 
\end{tabular}
\linebreak
\linebreak
...
Which $d_{v\alpha\beta}$ and $l_{v\alpha\beta}$ has a value will be determined by the binary variable $b_{v\alpha\beta}$ and the distance insterval parameter $B_{v\alpha}$ and the weight interval parameter $Z_{v\beta}$.  
\linebreak
...
\begin{alignat} {2}
    ...
    B_{v(\alpha-1)}b_{v\alpha\beta} \leq d_{v\alpha\beta} \leq 
    B_{v\alpha} b_{v\alpha\beta},				        	& 	&&	v \in V, (\alpha, \beta) \in A_v		\label{eq:23}	\\[8pt]
    Z_{v(\beta-1)}b_{v\alpha\beta} \leq l_{v\alpha\beta} \leq 
    Z_{v\beta} b_{v\alpha\beta},				        	& 	&&	v \in V, (\alpha, \beta) \in A_v		\label{eq:24}	\\[8pt]   	
