...
At a given point in time (could be delivery for one day or a week) we consider a planning problem with a demand for orders that should be satisfied with the given suppliers using vehicles provided by different logistic carriers. \par 
...
\begin{flalign*}
    ...
    V &- \text{vehicles}  \\
    E &- \text{edges} \\
    E_v &- \text{edges visitable by vehicle $v$} \\
    N_v &- \text{nodes visitable by vehicle $v$}  \\
    N^P &- \text{pickup nodes} \\
    N^D &- \text{delivery Nodes} \\
    F &- \text{factories} \\
    N_f &- \text{delivery nodes for factory $f$} \\
    S &- \text{cost structure} \\
    P_i &- \text{time windows at node $i$, $\{1,..,\pi_i \}$}\\
    T_{i} &- \text{timewindows at node $i$ $[ \underline{T_{ip}},  \overline{T_{ip}} ]$ where $p \in P_i$}
\end{flalign*}
...
\begin{flalign*}
    &Parameters \\ 
    n &- \text{amount of orders} \\
    K_v &- \text{weight limit of vehicle } v\in V \\
    Q_i &- \text{weight of order at node } i\in N\\
    H_f &- \text{docking limit at factory } f\in F\\
    T_{ijv} &- \text{travel time from node } i\in N \text{ to } j\in N \text{ for vehicle } v\in V\\
    \pi_i &- \text{amount of time windows at node } i\in N\\ 
    \overline{T_{ip}} &- \text{upper bound time of time window } p\in P_i \text{ at node } i\in N \\
    \underline{T_{ip}} &- \text{lower bound time of time window } p\in P_i \text{ at node } i\in N \\
    C_{vs} &- \text{cost per distance unit in structure interval } s \in S \text{ for vehicle } v \\
    C_i &- \text{cost of not transporting order at node } i\in N^P \\
    D_{ij} &- \text{distance between node $i\in N$ and } j\in N\\
    B_s &- \text{distance for interval $s\in S$ in cost structure}\\
\end{flalign*}
...
\begin{flalign*}
    &Variables\\
    x_{ijv} &- \text{binary indicating travel from node $i\in N$ to $j\in N$ of vehicle $v\in V$}\\
    y_i &- \text{binary indicating no pickup of order at node $i\in N^P$}\\
    l_{iv} &- \text{load of vehicle $v$ at node $i$}\\ 
    o(v) &- \text{origin node of vehicle $v$}\\
    d(v) &- \text{destination node of vehicle $v$}\\
    h_i &- \text{docking times in factory at node $i\in N_f$}\\
    t_i &- \text{time of visit at node $i\in N$}\\
    w_i &- \text{waiting time at node i}\\ 
    u_{ip} &- \text{binary indicating usage of time window $p\in P_i$ at node $i$}\\
    d_{vs} &- \text{total distance travelled of vehicle $v\in V$ if it fits in interval $s\in S$}\\
    b_{vs} &- \text{binary indicating correct interval $s\in S$ for vehicle $v\in V$}\\
\end{flalign*}
