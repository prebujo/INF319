...
set Beta = 1 .. nb;
set Beta_zero = 0 .. nb;
set A = {alp in Alpha, bet in Beta};

#Section of parameters
param o_v {v in V};
param d_v {v in V};
param C_km {v in V, (alp,bet) in A};
param C_kg {v in V, (alp,bet) in A};
param C_fix {v in V, (alp,bet) in A};
...
param B {alp in Alpha_zero};
param Z {bet in Beta_zero};

#Section of variables
var d {v in V, (alp,bet) in A} >= 0;
var y {i in N_p} binary;
var x {i in N, j in N, v in V:i<>j} binary;
var l {i in N, v in V} >= 0;
var l_max {v in V, (alp, bet) in A} >= 0;
var h {i in N} >= 0;
var u {i in N, p in P[i]} binary;
var t {i in N} >= 0;
var b {v in V, (alp,bet) in A} binary;

#objective function
minimize Total_cost: 
sum{v in V, (alp,bet) in A} (C_km[v,alp,bet] * d[v,alp,bet] + C_kg[v, alp, bet]*l_max[v, alp, bet] + C_fix[v, alp, bet]*b[v, alp, bet]) + 
...
subject to weight_pickup {v in V, j in N_vp[v], (i,j) in E[v]}: 
	l[i, v] + Q[j] - l[j,v] <= K[v] * (1 - x[i, j, v]);  

subject to weight_delivery {v in V, j in N_vp[v], (i,(j+nn)) in E[v]}: 
	l[i, v] - Q[j] - l[(j+nn),v] <= K[v] * (1 - x[i, (j+nn), v]);
...
subject to total_distance {v in V}: 
	sum {(alp,bet) in A} d[v,alp,bet] = sum {(i,j) in E[v]} x[i,j,v] * D[i,j];
	
subject to maximum_weight {v in V, i in N_v[v]}:
	sum {(alp,bet) in A} l_max[v, alp, bet] >= l[i, v];

subject to distance_interval_lower{v in V, (alp, bet) in A}:
	d[v, alp, bet] >= B[alp-1]*b[v, alp, bet];
	
subject to distance_interval_upper{v in V, (alp, bet) in A}:
	d[v, alp, bet] <= B[alp]*b[v, alp, bet];	
	
subject to weight_interval_lower{v in V, (alp, bet) in A}:
	l_max[v,alp,bet] >= Z[bet-1]*b[v, alp, bet];
	
subject to weight_interval_upper{v in V, (alp, bet) in A}:
	l_max[v,alp,bet] <= Z[bet]*b[v, alp, bet];
	
subject to matrix_selection{v in V}:
	sum{(alp,bet) in A} b[v,alp,bet] <= sum{j in N_v[v]: j<>o_v[v]&&j<>d_v[v]} x[o_v[v],j,v];
